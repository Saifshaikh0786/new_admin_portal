...
export default function SectionDetailView({ section, teachers = [], onBack, onStudentSelect, user, cache = {}, onUpdateCache }) {
    ...
    // Workflow helpers: avg practice progress per student (from already-fetched progressData)
    const [showAtRiskOnly, setShowAtRiskOnly] = useState(false);
    const [worstFirst, setWorstFirst] = useState(false);

    const getStudentAvgProgress = React.useCallback((student) => {
        if (!progressLoaded || !regularCourses.length) return null;
        let sum = 0, n = 0;
    ...
    }, [progressLoaded, progressData, regularCourses]);
    ...
    const sortedStudents = React.useMemo(() => {
        ...
        let registeredStudents = filtered.filter(s => s.student_name && s.student_name.trim() !== '');
        ...
        if (showAtRiskOnly) {
            registeredStudents = registeredStudents.filter(s => {
                const avg = getStudentAvgProgress(s);
                return avg !== null && avg < 40;
            });
        }
        if (worstFirst) {
            registeredStudents = [...registeredStudents].sort((a, b) => {
                const pa = getStudentAvgProgress(a); const pb = getStudentAvgProgress(b);
                return (pa === null ? 101 : pa) - (pb === null ? 101 : pb);
            });
        }

        return { registered: registeredStudents, unregistered: unregisteredStudents };
    }, [data, showAtRiskOnly, worstFirst, getStudentAvgProgress]);
    ...
    // Full-page skeleton only on FIRST load; search/sort/pagination refresh just the table
    if (loading && !data) {
    ...
    return (
        <PortalWrapper>
        <div className="fixed inset-0 z-[120] flex flex-col neu-page animate-in fade-in slide-in-from-right duration-300 overflow-hidden font-sans">
            {/* Background Effects */}
            <div className="absolute top-0 right-0 h-[500px] w-[500px] bg-gradient-to-br from-cyan-500/10 to-blue-500/10 blur-[100px] pointer-events-none transform-gpu opacity-50 dark:opacity-100" />
            <div className="absolute bottom-0 left-0 h-[500px] w-[500px] bg-gradient-to-tr from-violet-500/10 to-purple-500/10 blur-[100px] pointer-events-none transform-gpu opacity-30 dark:opacity-100" />

            {/* Header */}
            {!(sectionView === 'tests' && inspectingTest) && (
                <div className="relative flex items-center justify-between p-6 px-8 border-b border-[var(--neu-divider)] neu-raised backdrop-blur-xl shrink-0 z-20 transition-all duration-300">
                    <div className="flex items-center gap-6">
                    <button onClick={onBack} className="group p-3 rounded-2xl neu-raised hover:border-cyan-500/50 dark:hover:border-cyan-500/50 hover:shadow-lg hover:shadow-cyan-500/10 transition-all duration-300 active:scale-95">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400 font-medium flex-wrap">
                            {/* Total Students */}
                            <span className="flex items-center gap-1.5 px-3 py-1 neu-raised rounded-full shadow-sm">
                                <Users className="w-3.5 h-3.5 text-gray-500" />
                                {totalStudents} Total
                            </span>
                            {/* Registered */}
                            <span className="flex items-center gap-1.5 px-3 py-1 bg-[var(--neu-success-soft)] dark:bg-[var(--neu-success-soft)] rounded-full border border-emerald-200 dark:border-emerald-500/20 shadow-sm text-emerald-700 dark:text-[var(--neu-success)]">
                            ...
                            {/* Courses */}
                            <span className="flex items-center gap-1.5 px-3 py-1 neu-raised rounded-full shadow-sm">
                                <BookOpen className="w-3.5 h-3.5 text-[var(--neu-achieve)]" />
                                {course_performance?.length || 0} Courses
                            </span>
                            {/* Progress Loading Indicator */}
                            {loadingProgress && (
                                <span className="flex items-center gap-1.5 px-3 py-1 bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] rounded-full border border-blue-200 dark:border-blue-500/20 shadow-sm text-blue-700 dark:text-blue-400">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className={`group flex items-center gap-2 px-6 py-2.5 rounded-xl transition-all duration-300 shadow-lg hover:-translate-y-0.5 active:translate-y-0 font-bold text-sm tracking-wide ${sectionView === 'tests'
                            ? 'bg-[var(--neu-accent)] text-white shadow-violet-500/25 hover:shadow-violet-500/40'
                            : 'neu-raised text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)] border border-violet-200 dark:border-violet-500/20 hover:border-violet-400 hover:shadow-violet-500/20'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className="group flex items-center gap-2 px-6 py-2.5 neu-btn-primary"
                    >
            ...
            <div className="flex-1 overflow-hidden flex flex-col md:flex-row relative z-10">
                        ...
                        <div className="max-w-5xl mx-auto">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                                    <div className="p-2 bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] rounded-lg text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]">
                                        <Trophy className="w-5 h-5" />
                                    </div>
                                    Examinations
                                    <span className="text-sm font-normal text-gray-400 neu-inset px-2 py-0.5 rounded-md">{examCourses.length} exams</span>
                                </h3>
                                {examCourses.length > 0 && (
                                    <button
                                        onClick={handleExportAllTests}
                                        disabled={isExportingAllTests}
                                        className="group flex items-center gap-2 px-5 py-2.5 bg-[var(--neu-accent)] hover:from-violet-500 hover:to-purple-500 text-white rounded-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-violet-500/25 hover:shadow-violet-500/40 hover:-translate-y-0.5 active:translate-y-0 font-bold text-sm tracking-wide"
                                    >
                                ...
                                {examCourses.map(course => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className="text-left group relative overflow-hidden p-6 rounded-2xl bg-[var(--neu-achieve-soft)] border border-violet-200 dark:border-violet-500/20 shadow-sm transition-all duration-300 hover:shadow-xl hover:shadow-violet-500/15 hover:border-violet-400/50 dark:hover:border-violet-500/40 hover:scale-[1.02] hover:-translate-y-1"
                                        >
                                            {/* Decorative */}
                                            ...
                                            <div className="relative z-10">
                                                <div className="flex justify-between items-start mb-4">
                                                    <div className="w-12 h-12 rounded-xl bg-[var(--neu-accent)] flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform">
                                                        <Trophy className="w-6 h-6" />
                                                    </div>
                                                    <span className="px-2 py-0.5 rounded-full bg-[var(--neu-achieve-soft)] text-white text-[10px] font-bold uppercase tracking-wider shadow-sm">Exam</span>
                                                </div>

                                                <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-4 group-hover:text-[var(--neu-achieve)] dark:group-hover:text-violet-400 transition-colors">
                                                    {course.course_name}
                                                </h4>
                                                ...
                                                <div className="flex items-center text-sm text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)] font-bold group-hover:text-[var(--neu-achieve)] transition-colors">
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden">
                        ...
                        <div className="px-8 pt-6 pb-2 shrink-0">
                            <button
                                ...
                                className="inline-flex items-center gap-2 text-sm text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)] hover:text-[var(--neu-achieve)] font-bold transition-colors"
                            >
                ...
                {sectionView === 'students' && (
                    <>
                        ...
                        <div className="w-full md:w-[380px] p-6 border-r border-[var(--neu-divider)] neu-flat overflow-y-auto custom-scrollbar shrink-0 backdrop-blur-sm">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-sm font-extrabold uppercase tracking-widest text-gray-500 dark:text-gray-400 flex items-center gap-2">
                                    Course Performance
                                </h3>
                                <span className="px-2 py-0.5 neu-inset rounded-md text-[10px] font-bold text-gray-500 dark:text-gray-300">{regularCourses.length + examCourses.length} Courses</span>
                            </div>
                            ...
                            <div className="space-y-4">
                                {regularCourses.map(course => {
                                    ...
                                    return (
                                        <div key={course.course_id} className="group p-5 rounded-2xl neu-raised hover:border-cyan-500/30 dark:hover:border-cyan-500/30 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden">
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                {loadingProgress ? (
                                                    <div className="flex items-center gap-2 text-xs text-blue-500 bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] p-3 rounded-lg">
                                                    ...
                                                    <div className="space-y-3">
                                                        <div className="flex items-end justify-between">
                                                            <div>
                                                                ...
                                                                <span className="text-2xl font-bold neu-btn-primary">{avgProgress}%</span>
                                                            </div>
                                                        ...
                                                        <div className="h-1.5 w-full neu-inset rounded-full overflow-hidden">
                                                            <div className={`h-full rounded-full transition-all duration-1000 ${avgProgress > 75 ? 'bg-[var(--neu-success-soft)]' : avgProgress > 40 ? 'bg-[var(--neu-achieve-soft)]' : 'bg-orange-500'}`} style={{ width: `${avgProgress}%` }} />
                                                        </div>
                                                    </div>
                                                ) : progressLoaded ? (
                                                    <div className="flex items-center gap-2 text-xs text-gray-400 neu-inset p-2 rounded-lg">
                                ...
                                {/* EXAM COURSES */}
                                {examCourses.length > 0 && (
                                    <div className="mt-6 pt-6 border-t border-[var(--neu-divider)]">
                                        <h3 className="text-sm font-extrabold uppercase tracking-widest text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)] mb-4 flex items-center gap-2">
                                            Examinations
                                        </h3>
                                        <div className="space-y-3">
                                            {examCourses.map(course => (
                                                <button 
                                                    ...
                                                    className="w-full text-left p-5 rounded-2xl neu-chip neu-chip-accent neu-hover relative overflow-hidden group transition-all cursor-pointer"
                                                >
                                                    <div className="relative z-10 flex gap-3 items-start">
                                                        <div className="shrink-0 mt-0.5 w-4 h-4 rounded-full neu-inset flex items-center justify-center">
                        ...
                        {/* RIGHT: Student Table */}
                        <div className="flex-1 p-8 overflow-hidden flex flex-col neu-page">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-3">
                                    <div className="p-2 neu-chip neu-chip-accent rounded-lg">
                                ...
                                <div className="relative w-72 group">
                                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-hover:text-[var(--neu-achieve)] transition-colors" />
                                    <input
                                        type="text"
                                        placeholder="Search by name or Reg ID..."
                                        value={searchQuery}
                                        onChange={(e) => setSearchQuery(e.target.value)}
                                        className="w-full pl-10 pr-4 py-2.5 neu-inset rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500/50 text-sm text-gray-900 dark:text-white placeholder-gray-400 transition-all"
                                    />
                                    {searchQuery && (
                                        <button
                            ...
                            <div className="flex-1 overflow-hidden rounded-2xl neu-raised shadow-sm flex flex-col">
                                <div className="overflow-auto custom-scrollbar flex-1">
                                    <table className="w-full text-left border-collapse">
                                        <thead className="neu-table-head sticky top-0 backdrop-blur-md z-10">
                                            <tr>
                                                <th className="p-5 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-[var(--neu-dark)] transition-colors" onClick={() => requestSort('student_name')}>
                                                    <div className="flex items-center gap-2">
                                                        Student Name
                                                        <ArrowUpDown className={`w-3 h-3 ${sortConfig.key === 'student_name' ? 'text-cyan-500' : 'opacity-50'}`} />
                                                    </div>
                                                </th>
                                                <th className="p-5 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-[var(--neu-dark)] transition-colors" onClick={() => requestSort('uni_reg_id')}>
                                                    <div className="flex items-center gap-2">
                                                        Reg ID
                                                ...
                                                {progressLoaded && regularCourses.map(course => (
                                                    <th
                                                        key={course.course_id}
                                                        className="p-5 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center cursor-pointer hover:bg-[var(--neu-dark)] transition-colors group"
                                        ...
                                        <tbody className="divide-y divide-[var(--neu-divider)]">
                                            ...
                                            {sortedStudents.registered?.length > 0 && (
                                                <>
                                                    <tr className="bg-[var(--neu-success-soft)]">
                                                        <td colSpan={3 + (progressLoaded ? regularCourses.length : 0) + (loadingProgress ? 1 : 0)} className="p-3 px-5">
                                                            <div className="flex items-center justify-between gap-2">
                                                                <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[var(--neu-success)] dark:text-[var(--neu-success)]">
                                                                    <div className="w-2 h-2 bg-[var(--neu-success-soft)] rounded-full animate-pulse"></div>
                                                                    <UserCheck className="w-3.5 h-3.5" />
                                                                    Registered Students ({sortedStudents.registered.length})
                                                                </div>
                                                                {progressLoaded && regularCourses.length > 0 && (
                                                                    <div className="flex items-center gap-2 normal-case tracking-normal" onClick={(e) => e.stopPropagation()}>
                                                                        <button
                                                                            onClick={() => setWorstFirst(v => !v)}
                                                                            title="Show students with the lowest progress at the top"
                                                                            className={`neu-badge text-[11px] cursor-pointer ${worstFirst ? 'neu-badge-warn' : ''}`}>
                                                                            {worstFirst ? '✓ ' : ''}Lowest first
                                                                        </button>
                                                                        <button
                                                                            onClick={() => setShowAtRiskOnly(v => !v)}
                                                                            title="Show only students whose average progress is below 40%"
                                                                            className={`neu-badge text-[11px] cursor-pointer ${showAtRiskOnly ? 'neu-badge-danger' : ''}`}>
                                                                            {showAtRiskOnly ? '✓ ' : ''}Only below 40%
                                                                        </button>
                                                                    </div>
                                                                )}
                                                            </div>
                                                    ...
                                                    {sortedStudents.registered.map((student, idx) => (
                                                        <tr key={student.student_id || student.uni_reg_id}
                                                            ...
                                                            className="hover:bg-[var(--neu-achieve-soft)] transition-colors group cursor-pointer"
                                                        >
                                                            <td className="p-4 px-5">
                                                                <div className="flex items-center gap-3">
                                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-[var(--neu-shadow-tile)] ${['bg-[var(--neu-danger-soft)] text-[var(--neu-danger)]', 'bg-[var(--neu-achieve-soft)] text-blue-600', 'bg-green-100 text-green-600', 'bg-[var(--neu-warn-soft)] text-yellow-600', 'bg-[var(--neu-achieve-soft)] text-[var(--neu-achieve)]'][idx % 5]}`}>
                                                            ...
                                                            <td className="p-4 px-5">
                                                                <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-[var(--neu-success-soft)] text-emerald-700 dark:bg-[var(--neu-success-soft)] dark:text-[var(--neu-success)]">
                                                                    ...
                                                                    <td key={course.course_id} className="p-4 px-5 text-center">
                                                                        {studentProgress !== null ? (
                                                                            <div className="flex items-center justify-center gap-2">
                                                                                ...
                                                                                />
                                                                                <span className={`text-xs font-bold ${studentProgress > 80 ? 'text-[var(--neu-success)]' : 'text-gray-600 dark:text-gray-400'}`}>
                                                                                    {studentProgress}%
                                                                                </span>
                                                                            </div>
                                                                        ) : (
                                                ...
                                                <>
                                                    ...
                                                    {sortedStudents.unregistered.map((student, idx) => (
                                                        <tr key={student.student_id || student.uni_reg_id || `unreg-${idx}`} className="hover:bg-orange-50/30 dark:hover:bg-orange-500/5 transition-colors">
                                                            <td className="p-4 px-5">
                                                                <div className="flex items-center gap-3">
                                                                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold neu-inset text-gray-400 border-2 border-dashed border-gray-300 dark:border-gray-600">
                                                                        ?
                                                                    </div>
                                ...
                                {totalStudents > itemsPerPage && (
                                    <div className="flex items-center justify-between pt-4 mt-4 border-t border-[var(--neu-divider)]">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className="px-4 py-2 text-sm font-bold rounded-xl text-gray-700 dark:text-gray-300 hover:bg-[var(--neu-dark)] disabled:opacity-40 disabled:cursor-not-allowed transition-all active:scale-95"
                                            >
                                                Previous
                                            </button>
                                            <button
                    ...
                    <div className="relative w-full max-w-2xl max-h-[85vh] mx-4 bg-[#f0f4f8] dark:bg-[#0B0F19] rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
                        ...
                        <div className="flex items-center justify-between p-6 pb-4 border-b border-[var(--neu-divider)]">
                            ...
                            <button
                                ...
                                className="p-2 rounded-xl text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-[var(--neu-dark)] transition-all"
                            >
                                ...
                                {loadingStructures ? (
                                    ...
                                    <div className="space-y-4">
                                        {Object.entries(courseStructures).map(([courseId, courseData]) => {
                                            ...
                                            return (
                                                <div key={courseId} className="p-5 rounded-2xl neu-flat">
                        ...
                        {/* Modal Footer */}
                        <div className="p-6 pt-4 border-t border-[var(--neu-divider)] flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    className="px-6 py-2.5 rounded-xl text-sm font-bold text-white neu-btn-primary"
                                >
                                    Export Report
                                </button>
                            </div>
...
function TestDetailOverlay({ test, students, sectionMetadata, onClose, isInline = false, onStudentSelect }) {
    ...
    const stats = React.useMemo(() => {
        ...
        const attemptedMarks = marks.filter(m => m > 0);
        ...
        return {
            ...
            attemptedCount: attemptedMarks.length,
            avgScore: marks.length > 0 ? (marks.reduce((a, b) => a + b, 0) / marks.length).toFixed(1) : 0,
            avgAttempted: attemptedMarks.length > 0 ? (attemptedMarks.reduce((a, b) => a + b, 0) / attemptedMarks.length).toFixed(1) : 0,
    ...
    const renderSortIcon = (field) => {
        ...
        return sortConfig.direction === 'asc' ? <ChevronUp className="w-3 h-3 text-[var(--neu-achieve)]" /> : <ChevronDown className="w-3 h-3 text-[var(--neu-achieve)]" />;
    };

    const getRankBadge = (rank) => {
        if (rank === 1) return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[var(--neu-warn)] text-white text-xs font-black shadow-lg shadow-amber-500/40"><Trophy className="w-3.5 h-3.5" /> 1st</span>;
        if (rank === 2) return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[var(--neu-dark)] text-gray-800 text-xs font-black shadow-md"><Medal className="w-3.5 h-3.5" /> 2nd</span>;
        if (rank === 3) return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[var(--neu-warn)] text-white text-xs font-black shadow-md"><Medal className="w-3.5 h-3.5" /> 3rd</span>;
        const top10Cutoff = Math.ceil(rankedStudents.length * 0.1);
        if (rank <= top10Cutoff && rankedStudents.length >= 10) return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] text-violet-700 dark:text-violet-300 text-xs font-bold"><Award className="w-3 h-3" /> Top 10%</span>;
    ...
    const wrapperClass = isInline
        ...
        : 'fixed inset-0 z-[70] flex flex-col neu-flat animate-in slide-in-from-right duration-300 backdrop-blur-sm';

    return (
        <div className={wrapperClass}>
            {/* Header */}
            <div className={`flex items-center justify-between p-6 px-8 border-b border-[var(--neu-divider)] shrink-0 ${isInline ? 'neu-flat' : 'bg-[var(--card)] shadow-lg dark:bg-[#0f1523]'}`}>
                <div className="flex items-center gap-6">
                    {!isInline && (
                        <button onClick={onClose} className="p-3 neu-inset rounded-2xl hover:border-violet-500/30 text-gray-500 dark:text-gray-400 hover:text-[var(--neu-achieve)] dark:hover:text-violet-400 transition-all hover:scale-105 active:scale-95 shadow-sm">
                            <ArrowLeft className="w-5 h-5" />
                        </button>
                    )}
                    <div>
                        <div className="text-xs text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)] uppercase tracking-wider font-extrabold mb-1 flex items-center gap-2">
                        ...
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-1 flex items-center gap-3">
                            ...
                            <span className="text-sm font-normal text-gray-400 neu-inset px-2 py-0.5 rounded-md">{examStudents.length} Students</span>
                        </h2>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="pl-10 pr-4 py-2.5 neu-inset rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 text-sm text-gray-900 dark:text-white placeholder-gray-400 transition-all w-56"
                        />
                        {searchQuery && (
                    ...
                    <button
                        onClick={handleExport}
                        className="flex items-center gap-2 px-6 py-2.5 bg-[var(--neu-accent)] hover:from-violet-500 hover:to-purple-500 text-white rounded-xl transition-all shadow-lg shadow-violet-500/25 hover:shadow-violet-500/40 hover:-translate-y-0.5 active:translate-y-0"
                    >
                ...
                <div className="max-w-7xl mx-auto space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                        {[
                            ...
                            { label: `Avg (${stats.attemptedCount} Attempted)`, value: stats.avgAttempted, icon: TrendingUp, color: 'from-fuchsia-500 to-pink-500', iconColor: 'text-fuchsia-400' },
                            ...
                            <div key={i} className="group p-4 rounded-2xl neu-raised hover:border-violet-500/30 dark:hover:border-violet-500/30 transition-all duration-300 shadow-sm hover:shadow-md">
                    ...
                    <div className="neu-raised rounded-3xl overflow-hidden shadow-xl">
                        <table className="w-full text-left border-collapse">
                            <thead className="neu-flat border-b border-[var(--neu-divider)]">
                                <tr>
                                    <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-[var(--neu-dark)] transition-colors w-20" onClick={() => requestSort('rank')}>
                                        <div className="flex items-center gap-1.5"><Hash className="w-3 h-3" /> Rank {renderSortIcon("rank")}</div>
                                    </th>
                                    <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-[var(--neu-dark)] transition-colors" onClick={() => requestSort('student_name')}>
                                    ...
                                    <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center cursor-pointer hover:bg-[var(--neu-dark)] transition-colors" onClick={() => requestSort('total_marks')}>
                                        <div className="flex items-center justify-center gap-1.5">Total {renderSortIcon("total_marks")}</div>
                                    </th>
                                    <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center cursor-pointer hover:bg-[var(--neu-dark)] transition-colors" onClick={() => requestSort('coding_marks')}>
                                        <div className="flex items-center justify-center gap-1.5">Coding {renderSortIcon("coding_marks")}</div>
                                    </th>
                                    <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center cursor-pointer hover:bg-[var(--neu-dark)] transition-colors" onClick={() => requestSort('mcq_marks')}>
                                        <div className="flex items-center justify-center gap-1.5">MCQ {renderSortIcon("mcq_marks")}</div>
                                    </th>
                                    <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center cursor-pointer hover:bg-[var(--neu-dark)] transition-colors" onClick={() => requestSort('completion')}>
                                        <div className="flex items-center justify-center gap-1.5">Status {renderSortIcon("completion")}</div>
                                    </th>
                                    <th className="p-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center cursor-pointer hover:bg-[var(--neu-dark)] transition-colors" onClick={() => requestSort('duration')}>
                                ...
                                {displayStudents.map((student, i) => {
                                    ...
                                    return (
                                        ...
                                        <tr
                                            ...
                                            className={`transition-colors cursor-pointer ${!isRegistered ? 'opacity-50' : ''} ${student.rank <= 3 ? 'bg-[var(--neu-warn-soft)] to-transparent' : ''} hover:bg-[var(--neu-achieve-soft)] dark:hover:bg-[var(--neu-achieve-soft)]`}
                                        >
                                            {/* Rank */}
                                            ...
                                            <td className="p-4">
                                                {isRegistered ? (
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-[var(--neu-shadow-tile)] ${student.rank === 1 ? 'bg-[var(--neu-warn)] text-white shadow-lg shadow-amber-500/30' : student.rank <= 3 ? 'bg-[var(--neu-accent)] text-white' : 'neu-inset text-gray-600 dark:text-gray-300'}`}>
                                                            {student.student_name[0]?.toUpperCase() || 'S'}
                                                        </div>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs neu-inset text-gray-400 border border-dashed border-gray-300 dark:border-gray-600">?</div>
                                            ...
                                            <td className="p-4 text-center">
                                                <div className="flex flex-col items-center gap-1">
                                                    <span className={`text-lg font-black ${student.rank <= 3 ? 'bg-[var(--neu-accent)] bg-clip-text text-transparent' : 'text-gray-900 dark:text-white'}`}>
                                                        {student.total_marks}
                                                    </span>
                                                    <div className="h-1 w-16 neu-inset rounded-full overflow-hidden">
                                                        <div className={`h-full rounded-full transition-all duration-700 ${scorePercent > 80 ? 'bg-[var(--neu-success-soft)]' : scorePercent > 50 ? 'bg-[var(--neu-achieve-soft)]' : 'bg-orange-500'}`} style={{ width: `${scorePercent}%` }} />
                                                    </div>
                                                </div>
                                            </td>
                                            {/* Coding */}
                                            ...
                                            <td className="p-4 text-center">
                                                <div className="flex flex-col items-center gap-1">
                                                    <span className="text-sm font-bold text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]">{student.mcq_marks}</span>
                                                    <div className="h-1 w-12 neu-inset rounded-full overflow-hidden">
                                                        <div className="h-full bg-[var(--neu-achieve-soft)] rounded-full" style={{ width: `${Math.round((student.mcq_marks / mcqMax) * 100)}%` }} />
                                                    </div>
                                                </div>
                                            </td>
                                            {/* Completion */}
                                            <td className="p-4 text-center">
                                                <span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-sm ${student.exam_completion_percentage === 100
                                                    ? 'bg-[var(--neu-success-soft)] text-emerald-700 dark:bg-[var(--neu-success-soft)] dark:text-emerald-300'
                                                    : student.exam_completion_percentage > 0
                                                        ? 'bg-[var(--neu-warn-soft)] text-yellow-700 dark:bg-[var(--neu-warn-soft)] dark:text-yellow-300'
                                            ...
                                            <td className="p-4 text-center">
                                                <div className="flex flex-col items-center gap-1.5">
                                                    {student.mcq_duration !== null ? (
                                                        <span className="text-[10.5px] font-semibold text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)] bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] px-2 py-0.5 rounded-full whitespace-nowrap shadow-sm border border-violet-100 dark:border-violet-500/20">
                                                            MCQ: {formatDuration(student.mcq_duration)}
                                                        </span>
                                                    ) : null}
                                                    {student.coding_duration !== null ? (
                                                        <span className="text-[10.5px] font-semibold text-cyan-600 dark:text-cyan-400 bg-cyan-50 dark:bg-[var(--neu-achieve-soft)] px-2 py-0.5 rounded-full whitespace-nowrap shadow-sm border border-cyan-100 dark:border-cyan-500/20">
                                                            COD: {formatDuration(student.coding_duration)}
                                                        </span>
                                                    ) : null}
                                                    {student.mcq_duration === null && student.coding_duration === null && student.duration !== null ? (
                                                        <span className="text-[10.5px] font-semibold text-gray-700 dark:text-gray-300 neu-inset px-2 py-0.5 rounded-full whitespace-nowrap shadow-sm">
                                            ...
                                            <td className="p-4 text-center">
                                                <div className="flex flex-col items-center gap-1.5">
                                                    {student.startTime ? (
                                                        <span className="text-[10.5px] font-semibold text-gray-600 dark:text-gray-300 neu-inset px-2 py-0.5 rounded-full whitespace-nowrap shadow-sm">
                                                            STARTED: {new Date(student.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                        </span>
                                                    ) : null}
                                    ...
                                    <tr>
                                        <td colSpan={9} className="p-12 text-center">
                                            <div className="flex flex-col items-center gap-3 text-gray-400">
                                                ...
                                                <button onClick={() => setSearchQuery('')} className="text-xs text-[var(--neu-achieve)] hover:text-[var(--neu-achieve)] underline">Clear search</button>
                                            </div>
                                        </td>
                                    </tr>
                                )}
