...
export default function StudentDetailView({ student, onBack, onStudentSelect }) {
    ...
    return (
        <PortalWrapper>
        <div className="fixed inset-0 z-[120] flex flex-col overflow-hidden animate-in fade-in slide-in-from-right duration-300 neu-page">
            {/* Background Effects */}
            <div className="fixed -top-40 -left-48 h-[38rem] w-[38rem] bg-[var(--neu-achieve-soft)] blur-3xl rounded-full pointer-events-none opacity-50 dark:opacity-100" />
            ...
            {/* Header */}
            <div className="relative flex items-center gap-6 p-8 border-b border-[var(--neu-divider)] neu-raised backdrop-blur-xl sticky top-0 z-10 shadow-lg overflow-hidden">
                ...
                <button
                    ...
                    className="group p-3 rounded-xl neu-inset hover:from-blue-500 hover:to-indigo-500 dark:hover:from-blue-600 dark:hover:to-indigo-600 text-gray-700 dark:text-gray-300 hover:text-white transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-blue-500/30 hover:scale-110 z-10"
                >
                ...
                <div className="flex-1 z-10">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-12 h-12 rounded-xl bg-[var(--neu-accent)] flex items-center justify-center text-white font-bold text-xl shadow-lg ring-2 ring-white/10">
                            {(student.name || student.student_name || 'S')[0].toUpperCase()}
                        </div>
                        <h2 className="text-3xl font-bold bg-[var(--neu-accent)] bg-clip-text text-transparent flex items-center gap-3">
                            {student.name || student.student_name}
                        </h2>
                        <span className="px-4 py-1.5 rounded-full bg-[var(--neu-achieve-soft)] text-cyan-700 dark:text-cyan-400 border-2 border-cyan-500/30 font-mono text-sm font-bold shadow-md">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-sm font-medium flex items-center gap-2">
                        {((student.batch_name || student.batch) || (student.batch_id && student.batch_id.length < 10)) && (
                            <>
                                <span className="px-2 py-0.5 rounded-md bg-[var(--neu-achieve-soft)] dark:bg-violet-900/30 text-violet-700 dark:text-[var(--neu-achieve)] text-xs font-semibold">
                        ...
                        {examCourses.length > 0 && (
                            <>
                                <span className="text-gray-400">•</span>
                                <span className="px-2 py-0.5 rounded-md bg-[var(--neu-achieve-soft)] dark:bg-violet-900/30 text-violet-700 dark:text-[var(--neu-achieve)] text-xs font-bold">
                ...
                {viewLink === 'exam_deep_dive' && selectedExamCourse && (
                    <div className="flex flex-col h-full overflow-y-auto custom-scrollbar">
                        ...
                        {selectedExamCourse.examData?.myResult && (
                            <div className="shrink-0 mx-8 mt-6 p-4 bg-[var(--neu-achieve-soft)] dark:via-[#1A1F2E] dark:to-purple-900/20 rounded-2xl border border-violet-200 dark:border-violet-500/20 shadow-md">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-6">
                                        <div className="flex items-center gap-2">
                                            <Trophy className="w-4 h-4 text-[var(--neu-achieve)]" />
                                            <span className="text-xs text-[var(--neu-achieve)] uppercase font-bold tracking-wider">Your Exam Performance</span>
                                        </div>
                                        <div className="flex items-center gap-6">
                                            ...
                                            {selectedExamCourse.examData.rank && (
                                                <div className="text-center">
                                                    <div className={`text-2xl font-black ${selectedExamCourse.examData.rank <= 3 ? 'bg-[var(--neu-warn)] bg-clip-text text-transparent' : 'text-gray-700 dark:text-gray-200'}`}>
                                                        #{selectedExamCourse.examData.rank}
                                                    </div>
                                            ...
                                            <div className="text-center">
                                                <div className="text-lg font-black text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]">
                        ...
                        {selectedExamCourse.examData?.myResult?.analytics && (() => {
                            ...
                            return (
                                <div className="shrink-0 mx-8 mt-4 neu-raised rounded-2xl shadow-sm overflow-hidden transition-all duration-300">
                                    <div 
                                        onClick={() => setShowExamAnalytics(!showExamAnalytics)}
                                        role="button"
                                        tabIndex={0}
                                        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setShowExamAnalytics(!showExamAnalytics); }}
                                        className="w-full flex items-center justify-between p-5 hover:bg-[var(--neu-dark)] transition-colors group focus:outline-none cursor-pointer"
                                    >
                                        <div className="flex items-center gap-2">
                                            <Activity className="w-4 h-4 text-[var(--neu-achieve)] group-hover:scale-110 transition-transform" />
                                    ...
                                    <div className="flex items-center gap-2">
                                        <a
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                try {
                                                    sessionStorage.setItem('detailsAnalysisCtx', JSON.stringify({
                                                        studentId: fullStudent.student_id || fullStudent.uuid || fullStudent.uni_reg_id || fullStudent.reg_id,
                                                        courseId: selectedExamCourse.course_id,
                                                        analytics: selectedExamCourse.examData?.myResult?.analytics || null,
                                                        totalMarks: selectedExamCourse.examData?.myResult?.total_marks || 0,
                                                        marksBreakdown: selectedExamCourse.examData?.myResult?.marks_breakdown || {},
                                    ...
                                    {showExamAnalytics && (
                                        ...
                                        <div className="p-3 rounded-xl neu-flat border border-gray-100 dark:border-white/5">
                                            ...
                                            <div className={`text-lg font-black ${(an.lostFocusCount || 0) > 0 ? 'text-red-500' : 'text-[var(--neu-success)]'}`}>{an.lostFocusCount ?? 0}</div>
                                        </div>
                                        <div className="p-3 rounded-xl neu-flat border border-gray-100 dark:border-white/5">
                                            ...
                                            <div className={`text-lg font-black ${(an.faceWarnings || 0) > 0 ? 'text-amber-500' : 'text-[var(--neu-success)]'}`}>
                                        ...
                                        <div className="p-3 rounded-xl neu-flat border border-gray-100 dark:border-white/5">
                                            ...
                                            <div className={`text-lg font-black ${(an.internetDisconnects || 0) > 0 ? 'text-red-500' : 'text-[var(--neu-success)]'}`}>{an.internetDisconnects ?? 0}</div>
                                        ...
                                        <div className="p-3 rounded-xl neu-flat border border-gray-100 dark:border-white/5">
                                            ...
                                            <div className={`text-lg font-black ${(an.blockedByProctorCount || 0) > 0 ? 'text-red-500' : 'text-[var(--neu-success)]'}`}>{an.blockedByProctorCount ?? 0}</div>
                                        ...
                                        <div className="p-3 rounded-xl neu-flat border border-gray-100 dark:border-white/5">
                                            ...
                                            <div className="text-lg font-black text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]">{an.submitClicks ?? 0}</div>
                                        </div>
                                        <div className="p-3 rounded-xl neu-flat border border-gray-100 dark:border-white/5">
                                        ...
                                        <div className="p-3 rounded-xl neu-flat border border-gray-100 dark:border-white/5">
                                            ...
                                            <div className={`text-xs font-bold ${an.submitReason?.includes('student') ? 'text-[var(--neu-success)] dark:text-[var(--neu-success)]' : 'text-[var(--neu-warn)] dark:text-[var(--neu-warn)]'}`}>
                                                {an.submitReason || '-'}
                                            </div>
                                        </div>
                                        <div className="p-3 rounded-xl neu-flat border border-gray-100 dark:border-white/5">
                                    ...
                                    {an.perQuestion && Object.keys(an.perQuestion).length > 0 && (
                                        <div className="mt-4">
                                            <div className="flex items-center gap-2 mb-3">
                                                <MousePointerClick className="w-3.5 h-3.5 text-[var(--neu-achieve)]" />
                                                <span className="text-[10px] font-extrabold uppercase tracking-widest text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]">Per-Question Clicks</span>
                                            </div>
                                            <div className="rounded-xl overflow-hidden neu-flat">
                                                <table className="w-full text-xs">
                                                    <thead className="neu-inset border-b border-[var(--neu-divider)]">
                                                        <tr>
                                                            ...
                                                            <th className="p-2.5 text-center text-[10px] font-bold uppercase tracking-wider text-[var(--neu-achieve)]">Submit</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody className="divide-y divide-[var(--neu-divider)]">
                                                        {Object.entries(an.perQuestion).map(([qId, qData], qi) => (
                                                            <tr key={qId} className="hover:bg-[var(--card)]/50 dark:hover:bg-[var(--card)]/5 transition-colors">
                                                                ...
                                                                <td className="p-2.5 text-center font-bold text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]">{qData.submitClicks ?? 0}</td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                                </table>
                        ...
                        <div className="flex flex-1 min-h-[800px]">
                            {/* LEFT COLUMN */}
                            <div className="w-1/3 min-w-[350px] border-r border-[var(--neu-divider)] overflow-y-auto custom-scrollbar neu-flat backdrop-blur-md p-6">
                                <div className="mb-6">
                                    <div className="text-xs text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)] uppercase tracking-wider mb-2 flex items-center gap-1.5"><Trophy className="w-3 h-3" /> Exam Course</div>
                                    <h3 className="text-2xl font-bold leading-tight text-gray-900 dark:text-white mb-4">{selectedExamCourse.course_name}</h3>
                                    <div className="flex items-center gap-3 p-3 neu-raised rounded-xl">
                                    ...
                                    <div className="flex neu-inset p-1 rounded-lg mt-4 w-full">
                                        <button onClick={() => setProgressMode('attempts')} className={`flex-1 p-2 rounded-md text-xs font-medium transition-colors ${progressMode === 'attempts' ? 'neu-raised text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}>Completion (Attempted)</button>
                                        <button onClick={() => setProgressMode('marks')} className={`flex-1 p-2 rounded-md text-xs font-medium transition-colors ${progressMode === 'marks' ? 'neu-raised text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}>Completion (Marks)</button>
                                    </div>
                                ...
                                {loadingStructure ? (
                                    ...
                                    <div className="space-y-3">
                                        {courseStructure && Array.isArray(courseStructure) && courseStructure.map((unit) => (
                                            <div key={unit.unit_id} className="rounded-xl neu-raised overflow-hidden transition-all">
                                                <button
                                                    onClick={() => toggleUnit(unit.unit_id)}
                                                    className="w-full flex items-center justify-between p-4 hover:bg-[var(--neu-dark)] transition-colors"
                                                >
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-2 h-2 rounded-full shadow-[0_0_8px] ${selectedUnit === unit.unit_id ? 'bg-[var(--neu-achieve-soft)] shadow-violet-500/60' : 'bg-gray-400 dark:bg-gray-600 shadow-transparent'}`} />
                                                ...
                                                {selectedUnit === unit.unit_id && (
                                                    <div className="neu-flat border-t border-gray-100 dark:border-white/5 p-2 space-y-1">
                                                        {unit.sub_units && unit.sub_units.map((sub) => {
                                                            ...
                                                            return (
                                                                ...
                                                                className={`w-full text-left p-3 rounded-lg text-xs transition-all flex justify-between items-center group
                                                                    ${inspectingSubUnit?.subUnitId === sub.sub_unit_id
                                                                        ? 'bg-[var(--neu-achieve-soft)] text-violet-700 dark:text-[var(--neu-achieve)] border border-violet-500/70 font-bold shadow-[0_0_15px_rgba(139,92,246,0.15)]'
                                                            ...
                                                            >
                                                                <div className="flex items-center gap-2">
                                                                    <span>{sub.title}</span>
                                                                    {progress === 100 && (
                                                                        <Check className="w-3 h-3 text-[var(--neu-success)]" />
                                                                    )}
                                                                </div>
                                                                <div className="flex items-center gap-3">
                                                                    {breakdown && (
                                                                    ...
                                                                    {inspectingSubUnit?.subUnitId === sub.sub_unit_id && <div className="w-2 h-2 rounded-full bg-[var(--neu-achieve-soft)] shadow-[0_0_8px_rgba(139,92,246,0.8)]" />}
                                                                </div>
                                                            </button>
                                                            );
                                                        })}
                            ...
                            <div className="flex-1 p-8 overflow-y-auto custom-scrollbar relative">
                                {inspectingSubUnit ? (
                                    ...
                                    <div className="h-full flex flex-col items-center justify-center text-center text-gray-500">
                                        <div className="w-24 h-24 rounded-full bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] flex items-center justify-center mb-6 animate-pulse border border-violet-200 dark:border-violet-500/20">
                                            <Trophy className="w-10 h-10 opacity-30 text-[var(--neu-achieve)]" />
                                        </div>
                    ...
                    <div className="flex h-full">
                                                    ...
                                                    {unit.sub_units && unit.sub_units.map((sub) => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                className={`w-full text-left p-3 rounded-lg text-xs transition-all flex justify-between items-center group
                                                                    ${inspectingSubUnit?.subUnitId === sub.sub_unit_id
                                                                        ? 'bg-[var(--neu-achieve-soft)] text-blue-700 dark:text-blue-400 border border-blue-500/70 font-bold shadow-[0_0_15px_rgba(59,130,246,0.15)]'
                                                                        : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-[var(--card)] dark:hover:bg-[var(--card)]/5 border border-transparent'}`
                                                                }
                                                            >
                                                                <div className="flex items-center gap-2">
                                                                ...
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    {inspectingSubUnit?.subUnitId === sub.sub_unit_id && <div className="w-2 h-2 rounded-full bg-[var(--neu-achieve-soft)] shadow-[0_0_8px_rgba(59,130,246,0.8)]" />}
                                                                </div>
                                                            </button>
                                                        );
                                                    })}
                        ...
                        <div className="flex-1 p-8 overflow-y-auto custom-scrollbar relative">
                            {inspectingSubUnit ? (
                                ...
                                <div className="h-full flex flex-col items-center justify-center text-center text-gray-500">
                                    <div className="w-24 h-24 rounded-full neu-inset flex items-center justify-center mb-6 animate-pulse">
    ...
    return (
        <div className="p-8 overflow-y-auto h-full custom-scrollbar">
            ...
            <h3 className="text-xl font-bold mb-6 flex items-center gap-2 text-gray-900 dark:text-white">
                ...
                Practice Courses
                <span className="text-sm font-normal text-gray-400 neu-inset px-2 py-0.5 rounded-md">{courses.length}</span>
            </h3>
            {courses.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {courses.map((course, idx) => (
                        <button
                            ...
                            className="text-left group relative overflow-hidden p-6 rounded-2xl neu-raised hover:border-cyan-500/30 hover:bg-gray-50 dark:hover:bg-[var(--card)]/10 transition-all duration-300 backdrop-blur-sm"
                        >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                >
                                    ...
                                    <div className="relative z-10">
                                        <div className="flex justify-between items-start mb-4">
                                            <div className="w-12 h-12 rounded-xl neu-chip neu-chip-warn flex items-center justify-center">
                                                <Trophy className="w-6 h-6" />
                                            </div>
                                            <div className="flex flex-col items-end gap-1">
                                                <span className="neu-badge neu-badge-achieve text-[10px] uppercase tracking-wider">Exam</span>
                                                {course.course_code && (
                                                    <span className="text-[10px] font-mono text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]">{course.course_code}</span>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {myResult ? (
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    {rank && (
                                                        <div className="text-right">
                                                            ...
                                                            <span className={`text-xl font-black ${rank <= 3 ? 'text-[var(--neu-warn)]' : 'text-gray-700 dark:text-gray-300'}`}>
                                                                #{rank}
                                                ...
                                                <div className="flex gap-3">
                                                    <div className="flex-1 neu-chip neu-chip-info rounded-lg p-2 text-center flex-col">
                                                        <div className="text-[10px] font-bold uppercase">Coding</div>
                                                        <div className="text-sm font-bold text-gray-900 dark:text-white">{myResult.marks_breakdown?.coding_marks || 0}</div>
                                                    </div>
                                                    <div className="flex-1 neu-chip neu-chip-accent rounded-lg p-2 text-center flex-col">
                                                        <div className="text-[10px] text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)] font-bold uppercase">MCQ</div>
                                            ...
                                            <div className="flex items-center gap-2 text-sm text-gray-400 neu-inset p-3 rounded-lg">
                                        ...
                                        {/* CTA */}
                                        <div className="mt-4 pt-3 border-t border-[var(--neu-divider)] flex items-center text-sm font-semibold text-[var(--neu-accent)] transition-colors">
        ...
        <div className="flex flex-col h-full overflow-auto custom-scrollbar">
            <div className="p-8 space-y-6">
                ...
                <div className="flex items-center gap-4">
                    <button onClick={onBack} className="p-2 rounded-xl neu-raised hover:border-violet-500/30 text-gray-500 hover:text-[var(--neu-achieve)] transition-all shadow-sm">
                        <ArrowLeft className="w-5 h-5" />
                    </button>
                    <div>
                        <div className="text-xs text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)] uppercase tracking-wider font-bold flex items-center gap-1.5"><Trophy className="w-3 h-3" /> Exam Results</div>
                ...
                {myResult ? (
                    <div className="bg-gradient-to-br from-violet-50 via-white to-purple-50 dark:from-violet-900/20 dark:via-[#1A1F2E] dark:to-purple-900/20 rounded-3xl border border-violet-200 dark:border-violet-500/20 p-8 shadow-lg">
                        <div className="text-xs text-[var(--neu-achieve)] uppercase font-bold tracking-wider mb-4">Your Performance</div>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {/* Score */}
                            <div className="text-center">
                                <div className="text-4xl font-black bg-[var(--neu-accent)] bg-clip-text text-transparent">
                            ...
                            {/* Rank */}
                            <div className="text-center">
                                <div className={`text-4xl font-black ${rank <= 3 ? 'bg-[var(--neu-warn)] bg-clip-text text-transparent' : 'text-gray-700 dark:text-gray-200'}`}>
                                    #{rank}
                                </div>
                            ...
                            {/* MCQ */}
                            <div className="text-center">
                                <div className="text-3xl font-black text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]">
                        ...
                        <div className="mt-6 flex items-center gap-4">
                            <div className="flex-1 h-2 neu-inset rounded-full overflow-hidden">
                                <div className={`h-full rounded-full transition-all duration-700 ${myResult.exam_completion_percentage === 100 ? 'bg-[var(--neu-success-soft)]' : 'bg-[var(--neu-achieve-soft)]'}`} style={{ width: `${myResult.exam_completion_percentage || 0}%` }} />
                            </div>
                            <span className={`text-sm font-bold ${myResult.exam_completion_percentage === 100 ? 'text-[var(--neu-success)]' : 'text-[var(--neu-achieve)]'}`}>
                                {myResult.exam_completion_percentage || 0}% Complete
                            </span>
                        </div>
                    </div>
                ) : (
                    <div className="neu-raised rounded-2xl p-8 text-center text-gray-400">
                ...
                {leaderboard.length > 0 && (
                    <div className="neu-raised rounded-3xl overflow-hidden shadow-lg">
                        <div className="p-5 border-b border-[var(--neu-divider)] flex items-center justify-between">
                            <h4 className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2">
                                <Trophy className="w-4 h-4 text-amber-500" />
                                Section Leaderboard
                                <span className="text-xs font-normal text-gray-400 neu-inset px-2 py-0.5 rounded-md">{leaderboard.length} participants</span>
                            </h4>
                        </div>
                        <table className="w-full text-left border-collapse">
                            <thead className="neu-flat border-b border-[var(--neu-divider)]">
                                <tr>
                                ...
                                {leaderboard.map((s, i) => {
                                    ...
                                    return (
                                        <tr key={s.uni_reg_id || i}
                                            ...
                                            className={`transition-colors ${!isMe && onStudentSelect && s.student_name ? 'cursor-pointer' : ''} ${isMe ? 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] ring-1 ring-violet-400/30' : 'hover:bg-[var(--neu-dark)]'}`}
                                        >
                                            <td className="p-4 text-center">
                                                {s.rank === 1 ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--neu-warn)] text-white text-xs font-black shadow"><Trophy className="w-3 h-3" /> 1st</span>
                                                    : s.rank === 2 ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--neu-dark)] text-gray-800 text-xs font-black"><Medal className="w-3 h-3" /> 2nd</span>
                                                        : s.rank === 3 ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--neu-warn)] text-white text-xs font-black"><Medal className="w-3 h-3" /> 3rd</span>
                                            ...
                                            <td className="p-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {isMe && <span className="px-1.5 py-0.5 rounded text-[9px] font-black bg-[var(--neu-achieve-soft)] text-white uppercase">You</span>}
                                                </div>
                                                <div className="text-[10px] text-gray-400 font-mono">{s.uni_reg_id}</div>
                                            </td>
                                            <td className="p-4 text-center">
                                                <div className="flex flex-col items-center gap-1">
                                                    <span className={`text-base font-black ${isMe ? 'text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]' : 'text-gray-900 dark:text-white'}`}>{s.total_marks || 0}</span>
                                                    <div className="h-1 w-12 neu-inset rounded-full overflow-hidden">
                                                        <div className="h-full bg-[var(--neu-achieve-soft)] rounded-full" style={{ width: `${Math.round(((s.total_marks || 0) / maxMarks) * 100)}%` }} />
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-4 text-center text-sm font-bold text-cyan-600 dark:text-cyan-400">{s.marks_breakdown?.coding_marks || 0}</td>
                                            <td className="p-4 text-center text-sm font-bold text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]">{s.marks_breakdown?.mcq_marks || 0}</td>
                                            <td className="p-4 text-center">
                                                <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${s.exam_completion_percentage === 100 ? 'bg-[var(--neu-success-soft)] text-emerald-700 dark:bg-[var(--neu-success-soft)] dark:text-emerald-300' : s.exam_completion_percentage > 0 ? 'bg-[var(--neu-warn-soft)] text-yellow-700 dark:bg-[var(--neu-warn-soft)] dark:text-yellow-300' : 'bg-gray-100 text-gray-400 dark:bg-[var(--card)]/10'}`}>
        ...
        return (
            <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-right-8 duration-500">
                ...
                <div className="sticky top-0 z-30 flex justify-between items-center px-3 py-2 rounded-xl neu-raised backdrop-blur-xl shadow-lg">
                    ...
                    >
                        <div className="w-7 h-7 rounded-full neu-inset flex items-center justify-center group-hover:bg-gray-200 dark:group-hover:bg-[var(--card)]/10 transition-colors">
                    ...
                    {!loadingDetails && attemptDetails && (
                        <button
                            ...
                            className="neu-btn-primary px-4 py-1.5 rounded-lg text-sm font-bold transition-all hover:scale-105 flex items-center gap-2"
                        >
                ...
                {loadingDetails ? (
                    <div className="space-y-6">
                        <Skeleton className="h-64 w-full rounded-3xl neu-raised" />
                        <div className="grid grid-cols-3 gap-6">
                            {[1, 2, 3].map(i => <Skeleton key={i} className="h-32 rounded-2xl neu-raised" />)}
                        </div>
                    </div>
                ) : attemptDetails ? (
                    <div className="relative space-y-8">
                        {/* SCORE CARD - Fixed Layout */}
                        <div className="relative overflow-hidden rounded-3xl neu-raised shadow-2xl">
                            {/* Header Band — single compact row */}
                            <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 px-6 py-3.5">
                                <div className="flex items-center justify-between gap-4">
                                    <div className="flex items-center gap-3 flex-wrap">
                                        <h2 className="text-lg font-black text-white">Attempt Report</h2>
                                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${resultType === 'mcq' ? 'bg-blue-500/20 text-blue-300' : 'bg-emerald-500/20 text-emerald-300'}`}>
                                            {resultType === 'mcq' ? 'MCQ' : 'Coding'}
                                        </span>
                                        <span className="px-2.5 py-0.5 rounded-full bg-white/10 text-gray-300 text-[10px] font-bold uppercase tracking-wider">
                                            Attempt #{attemptDetails.overview.attempt_number}
                                        </span>
                                        {attemptDetails.overview.attempt_number === 1 && (
                                            <span className="px-2.5 py-0.5 rounded-full bg-violet-500/20 text-violet-300 text-[10px] font-bold uppercase tracking-wider">First Try</span>
                                        )}
                                    </div>

                                    {/* Pass/Fail */}
                                    <div className={`w-9 h-9 shrink-0 rounded-full flex items-center justify-center ${isPassed
                                        ? 'bg-emerald-500/20 shadow-[0_0_18px_rgba(16,185,129,0.35)]'
                                        : 'bg-red-500/20 shadow-[0_0_18px_rgba(239,68,68,0.35)]'
                                        }`}>
                                        {isPassed ? (
                            ...
                            <div className="p-6">
                                <div className="flex flex-col md:flex-row gap-6 items-center">
                                    ...
                                    <div className="relative shrink-0">
                                        <div className="relative neu-inset rounded-full p-3 shadow-xl">
                                            <CircularProgress
                                            ...
                                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                ...
                                                <span className={`text-xl font-black mt-7 ${isPassed ? 'text-[var(--neu-success)]' : 'text-red-500'}`}>
                        ...
                        {attemptDetails.proctoring_metrics && (
                            <div className="rounded-2xl neu-raised p-6 mb-4">
                        ...
                        <div className="space-y-4">
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                                {resultType === 'mcq' ? (
                                    ...
                                    <><Code className="w-5 h-5 text-[var(--neu-success)]" /> Coding Analysis</>
                                )}
                            </h3>
                            <div className="rounded-2xl overflow-hidden neu-raised">
        ...
        <div className="h-full flex flex-col relative z-0">
            <div className="mb-8 flex justify-between items-end">
                ...
                {!hasNeither && !hasOnlyOne && (
                    <div className="flex neu-inset p-1.5 rounded-xl">
                        {hasMCQ && (
                            <button
                                onClick={() => setResultType('mcq')}
                                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${resultType === 'mcq' ? 'neu-raised text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
                            >
                        ...
                        {hasCoding && (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${resultType === 'coding' ? 'neu-raised text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
                            >
                ...
                {hasOnlyOne && (
                    <div className="px-4 py-2 rounded-xl neu-inset text-gray-600 dark:text-gray-400 text-sm font-bold flex items-center gap-2">
            ...
            {hasNeither ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-12 border-2 border-dashed border-[var(--neu-divider)] rounded-3xl neu-flat">
                    <div className="w-20 h-20 neu-inset rounded-full flex items-center justify-center mb-6">
                ...
                <div className="space-y-8 pl-8 border-l-2 border-[var(--neu-divider)] ml-4 py-4">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="relative pl-8">
                            <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full neu-inset" />
                            <Skeleton className="h-24 w-full rounded-2xl bg-[var(--card)]/5" />
                        </div>
                    ))}
                </div>
            ) : history && history.length > 0 ? (
                <div className="relative pl-8 border-l-2 border-dashed border-[var(--neu-divider)] ml-4 py-4 space-y-10">
                    {history.map((attempt, idx) => {
                        ...
                        return (
                            <div key={idx} className="relative group perspective-1000">
                                <div className={`absolute -left-[41px] top-6 z-10 w-6 h-6 rounded-full border-4 border-gray-50 dark:border-[#0B0F19] transition-all duration-300 ${isRecent ? 'bg-[var(--neu-achieve-soft)] scale-110 shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'bg-gray-300 dark:bg-gray-700 group-hover:bg-blue-400'}`} />

                                <div className="neu-raised p-6 rounded-2xl transition-all duration-300 hover:border-blue-500/30 hover:shadow-2xl hover:shadow-blue-900/10 group-hover:-translate-y-1 group-hover:translate-x-1">
                                    <button
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-start relative z-10">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 rounded-xl neu-inset flex items-center justify-center font-bold text-gray-500 group-hover:bg-[var(--neu-achieve-soft)] group-hover:text-white transition-colors">
                                            ...
                                            <div className="flex flex-col items-end gap-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {isRecent && (
                                                        <span className="px-3 py-1 rounded-full bg-[var(--neu-achieve-soft)] border border-blue-500/20 text-blue-500 text-xs font-bold uppercase flex items-center gap-1">
                                                            <Zap className="w-3 h-3" /> Latest
                                                        </span>
                                                    )}
                                                    {/* Pass/Fail indicator */}
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isPassed
                                                        ? 'bg-[var(--neu-success-soft)] shadow-[0_0_15px_rgba(16,185,129,0.3)]'
                                                        : 'bg-[var(--neu-danger-soft)] shadow-[0_0_15px_rgba(239,68,68,0.3)]'
                                                        }`}>
                                                        {isPassed ? (
                                                            <Check className="w-4 h-4 text-[var(--neu-success)]" />
                                                        ) : (
...
const StatCard = ({ label, value, color = 'gray' }) => {
    const colorClasses = {
        emerald: 'text-[var(--neu-success)]',
        red: 'text-red-500',
        blue: 'text-blue-500',
        violet: 'text-[var(--neu-achieve)]',
        cyan: 'text-cyan-500',
        gray: 'text-gray-900 dark:text-white'
    };

    return (
        <div className="p-4 rounded-xl neu-inset border border-gray-100 dark:border-white/5">
...
const MetricCard = ({ label, value, isWarning }) => (
    <div className={`p-3 rounded-xl text-center ${isWarning ? 'bg-[var(--neu-danger-soft)] border border-red-500/20' : 'neu-inset border border-gray-100 dark:border-white/5'}`}>
...
const cleanQuestionText = (t) => (t || '').replace(/```[\s\S]*?```/g, ' ').replace(/[#*`>_|-]+/g, ' ').replace(/\s+/g, ' ').trim();

const SubmissionCard = ({ sub, idx, resultType, getQuestionScore, getQuestionMaxScore, analytics }) => {
    const [expanded, setExpanded] = useState(false);

    const title = cleanQuestionText(sub.question_title).slice(0, 120) || 'Question';
    const desc = cleanQuestionText(sub.question_desc);
    ...
    return (
        <div className="border-b last:border-0 border-gray-100 dark:border-white/5">
            <button
                ...
                className="w-full p-4 hover:bg-[var(--neu-dark)] transition-colors flex justify-between items-start text-left"
            >
                <div className="flex items-start gap-3 flex-1">
                    <span className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${isCorrect ? 'bg-[var(--neu-success-soft)] text-[var(--neu-success)] dark:text-[var(--neu-success)]' : 'bg-[var(--neu-danger-soft)] text-[var(--neu-danger)] dark:text-[var(--neu-danger)]'
                        }`}>
                        {idx + 1}
                    </span>
                ...
                <div className="flex items-center gap-3 shrink-0 ml-4">
                    <div className={`px-3 py-1 rounded-lg text-sm font-bold ${isCorrect ? 'bg-[var(--neu-success-soft)] text-[var(--neu-success)]' : 'bg-[var(--neu-danger-soft)] text-red-500'}`}>
                        {score} / {maxScore}
                    </div>
        ...
        <div className="flex flex-col font-mono bg-[#0B0F19] rounded-xl overflow-hidden border border-white/5">
            ...
            <div className="px-3 py-2 border-b border-white/5 bg-[var(--card)]/[0.02] flex flex-wrap gap-2 shrink-0">
                ...
                {config.proxy?.settings && (
                    <span className={`px-2 py-1 rounded flex items-center gap-1.5 border border-white/10 text-[10px] shadow-sm ${config.proxy.settings === 'DIRECT' ? 'bg-[var(--card)]/5 text-gray-300' : 'bg-[var(--neu-danger-soft)] text-red-400 border-red-500/30'}`}>
...
const DebugConfigCard = ({ title, config }) => {
    if (!config) return <div className="text-xs text-gray-500 p-4 border border-dashed border-[var(--neu-divider)] rounded-xl">No configuration logged</div>;
    return (
        <div className="neu-flat rounded-xl p-5 text-sm hover:border-blue-500/30 dark:hover:border-blue-500/30 transition-colors shadow-sm">
            ...
            <div className="space-y-6">
                ...
                {config.os && (
                    <div className="flex items-start gap-4">
                        <div className="w-10 h-10 rounded-xl bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] flex items-center justify-center shrink-0 border border-blue-100 dark:border-blue-500/20">
                        ...
                        <div className="w-full min-w-0">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                {Object.entries(config.os).map(([k, v]) => (
                                    <div key={k} className="flex justify-between items-center neu-raised p-2 rounded border border-gray-100 dark:border-white/5 overflow-hidden">
                ...
                {config.network && config.network.interfaces && config.network.interfaces.length > 0 && (
                    <div className="flex items-start gap-4">
                        <div className="w-10 h-10 rounded-xl bg-[var(--neu-success-soft)] dark:bg-[var(--neu-success-soft)] flex items-center justify-center shrink-0 border border-emerald-100 dark:border-emerald-500/20">
                            <Activity className="w-5 h-5 text-[var(--neu-success)]" />
                        </div>
                        <div className="w-full min-w-0">
                            <div className="text-[10px] uppercase font-bold tracking-wider text-gray-500 mb-2">Network Interfaces</div>
                            <div className="space-y-3 w-full">
                                {config.network.interfaces.map((intf, i) => (
                                    <div key={i} className="flex flex-col neu-raised p-3 rounded-lg border border-gray-100 dark:border-white/5 shadow-sm space-y-2">
                                         {Object.entries(intf).map(([k, v]) => (
                                            <div key={k} className="flex justify-between items-center text-xs overflow-hidden">
                                                <span className="text-[10px] text-gray-500 uppercase">{k}</span>
                                                <span className={`${k === 'ip' ? 'text-[var(--neu-success)] dark:text-[var(--neu-success)] font-mono' : 'text-gray-700 dark:text-gray-300'} truncate ml-2`} title={v}>{v}</span>
                                            </div>
                                         ))}
                                    </div>
                                ))}
                ...
                {config.proxy && (
                    <div className="flex items-start gap-4">
                        <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-[var(--neu-warn-soft)] flex items-center justify-center shrink-0 border border-amber-100 dark:border-amber-500/20">
                        ...
                        <div className="w-full min-w-0">
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                {Object.entries(config.proxy).map(([k, v]) => (
                                    <div key={k} className="flex justify-between items-center neu-raised p-2 rounded border border-gray-100 dark:border-white/5 overflow-hidden">
                                        <span className="text-[10px] text-gray-500 uppercase">{k}</span>
                                        <span className="text-xs font-mono text-[var(--neu-warn)] dark:text-[var(--neu-warn)] truncate ml-2" title={v}>{v}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="flex items-start gap-4 pt-4 border-t border-gray-100 dark:border-white/5">
                    <div className="w-10 h-10 rounded-xl bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] flex items-center justify-center shrink-0 border border-violet-100 dark:border-violet-500/20">
                        <Clock className="w-5 h-5 text-[var(--neu-achieve)]" />
                    </div>
                    <div className="w-full min-w-0">
...
const SubmissionDebugDropdown = ({ resultType, debugConfigs }) => {
    ...
    return (
        <div className="mt-6 rounded-xl overflow-hidden neu-raised">
            <button
                onClick={(e) => {
                    e.stopPropagation();
                    setIsOpen(!isOpen);
                }}
                className="w-full flex justify-between items-center p-4 text-sm font-bold text-gray-700 dark:text-gray-300 hover:bg-[var(--neu-dark)] transition-colors focus:outline-none"
            >
                <div className="flex items-center gap-2">
            ...
            {isOpen && (
                <div className="p-5 border-t border-[var(--neu-divider)] neu-flat cursor-default">
                    {resultType === 'mcq' ? (
                        ...
                        <div className="bg-[#0A0D14] rounded-xl border border-white/10 overflow-hidden shadow-inner flex flex-col xl:flex-row divide-y xl:divide-y-0 xl:divide-x divide-white/10">
                            <div className="flex-1 p-5 lg:p-6 hover:bg-[var(--card)]/[0.01] transition-colors">
                                <span className="text-[10px] text-cyan-500 font-mono uppercase font-black tracking-widest flex items-center gap-2 mb-4 bg-[var(--neu-achieve-soft)] w-fit px-3 py-1 rounded-full border border-cyan-500/20">
                            ...
                            <div className="flex-1 p-5 lg:p-6 bg-[var(--card)]/[0.02] hover:bg-[var(--card)]/[0.03] transition-colors">
                                <span className="text-[10px] text-[var(--neu-achieve)] font-mono uppercase font-black tracking-widest flex items-center gap-2 mb-4 bg-[var(--neu-achieve-soft)] w-fit px-3 py-1 rounded-full border border-violet-500/20">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-1 space-y-2">
                    ...
                    {sub.options && sub.options.map((option, optIdx) => {
                        ...
                        let bgClass = 'neu-inset border-gray-100 dark:border-white/5';
                        let textClass = 'text-gray-700 dark:text-gray-300';

                        if (isCorrectAnswer) {
                            bgClass = 'bg-[var(--neu-success-soft)] dark:bg-[var(--neu-success-soft)] border-emerald-200 dark:border-emerald-500/20';
                            textClass = 'text-emerald-700 dark:text-[var(--neu-success)]';
                        } else if (isSelected && !isCorrectAnswer) {
                            bgClass = 'bg-red-50 dark:bg-[var(--neu-danger-soft)] border-red-200 dark:border-red-500/20';
                            textClass = 'text-red-700 dark:text-[var(--neu-danger)]';
                        }
                            ...
                            <div key={optIdx} className={`p-3 rounded-lg border flex items-center justify-between ${bgClass}`}>
                                <div className="flex flex-col">
                                    ...
                                    <div className="flex gap-2 mt-1">
                                        ...
                                        {isCorrectAnswer && <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--neu-success)] dark:text-[var(--neu-success)]">(Correct Answer)</span>}
                                    </div>
                                </div>
                                {isCorrectAnswer && <Check className="w-5 h-5 text-[var(--neu-success)]" />}
                ...
                <div className="shrink-0 w-full md:w-56 flex flex-col items-center justify-center p-5 neu-flat rounded-2xl border border-gray-100 dark:border-white/5">
                    <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-3">Result</div>
                    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-3 ${correctPct === 100 ? 'bg-[var(--neu-success-soft)]' : correctPct === 0 ? 'bg-[var(--neu-danger-soft)]' : 'bg-amber-500/15'}`}>
                        {correctPct === 100 ? <Check className="w-8 h-8 text-[var(--neu-success)]" /> : correctPct === 0 ? <X className="w-8 h-8 text-red-500" /> : <span className="text-lg font-black text-amber-500">{correctPct}%</span>}
                    </div>
                    <div className={`text-sm font-black uppercase tracking-wider ${correctPct === 100 ? 'text-[var(--neu-success)]' : correctPct === 0 ? 'text-red-500' : 'text-amber-500'}`}>
                        {correctPct === 100 ? 'Correct' : correctPct === 0 ? 'Incorrect' : `${correctPct}% Correct`}
                    </div>
                    <div className="flex items-center gap-4 mt-5 text-[10px] font-bold uppercase tracking-wider">
                        <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-sm bg-[var(--neu-success-soft)] shadow-sm shadow-emerald-500/40"/>{correctCount} Right</div>
            ...
            {hasHistory && (
                <div className="space-y-4 pt-6 mt-4 border-t border-gray-100 dark:border-white/5">
                    <div className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                        <Clock className="w-4 h-4 text-[var(--neu-achieve)]" /> Submission Timeline
                    </div>
                        ...
                        {sub.history.map((h, i) => {
                            ...
                            return (
                                <div key={i} className="relative">
                                    <div className={`absolute -left-[1.375rem] top-2 w-3.5 h-3.5 rounded-full border-[3px] border-white dark:border-[#1A1F2E] ${h.is_correct ? 'bg-[var(--neu-success-soft)]' : 'bg-red-500'}`} />
                                    <div className={`p-4 rounded-xl border flex items-center justify-between transition-colors ${isLatest ? 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] border-blue-200 dark:border-blue-500/20' : 'neu-raised border-gray-100 dark:border-white/5'}`}>
                                        <div>
                                            <div className="flex items-center gap-3 mb-1.5">
                                                <span className={`text-xs font-black uppercase tracking-wider ${h.is_correct ? 'text-[var(--neu-success)] dark:text-[var(--neu-success)]' : 'text-[var(--neu-danger)] dark:text-[var(--neu-danger)]'}`}>
                                                    {h.is_correct ? 'Correct' : 'Incorrect'}
                                                </span>
                                                <span className="text-[10px] text-gray-400 font-mono tracking-wider">
                                                    {h.timestamp ? new Date(h.timestamp).toLocaleString() : 'N/A'}
                                                </span>
                                                {isLatest && <span className="text-[9px] bg-[var(--neu-achieve-soft)] text-white px-2 py-0.5 rounded-full font-bold uppercase tracking-widest shadow-sm shadow-blue-500/40 mt-[-1px]">Latest</span>}
                                            </div>
...
const CodingSubmissionDetail = ({ sub, analytics }) => {
    ...
    return (
        <div className="space-y-8 pl-11">
            ...
            {sub.question_desc && (
                <div className="neu-flat rounded-2xl border border-gray-100 dark:border-white/5 overflow-hidden">
                    <button
                        onClick={() => setShowQuestion(!showQuestion)}
                        className="w-full flex items-center justify-between p-4 hover:bg-[var(--neu-dark)] transition-colors"
                    >
                        <span className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                            <FileText className="w-4 h-4 text-violet-500" /> Full Question
                        </span>
                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${showQuestion ? 'rotate-180' : ''}`} />
                    </button>
                    {showQuestion && (
                        <div className="px-4 pb-4 space-y-3 animate-in fade-in slide-in-from-top-1 duration-200">
                            <div className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap leading-relaxed neu-inset p-4 rounded-xl">
                                {sub.question_desc}
                            </div>
                        </div>
                    )}
            ...
            <div className="neu-flat p-5 rounded-2xl border border-gray-100 dark:border-white/5">
                ...
                {totalClicks > 0 ? (
                    <div className="space-y-3">
                        <div className="flex items-center justify-between text-[11px] font-bold text-gray-500 uppercase tracking-wider">
                            ...
                            <span className="flex items-center gap-1.5 text-[var(--neu-achieve)] dark:text-[var(--neu-achieve)]"><div className="w-2 h-2 rounded bg-[var(--neu-achieve-soft)]" /> Submit Clicks ({qData.submitClicks || 0})</span>
                        </div>
                        <div className="flex h-3 text-white text-xs font-bold text-center rounded-full overflow-hidden shadow-inner">
                            ...
                            <div style={{ width: `${submitPct}%` }} className="bg-[var(--neu-achieve-soft)] flex items-center justify-center transition-all duration-500 {submitPct > 10 ? 'px-2' : ''}" title={`Submit: ${qData.submitClicks}`} />
                        </div>
                    </div>
                ) : (
            ...
            {sampleCases.length > 0 && (
                <div className="space-y-2">
                    <div className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                        <Eye className="w-4 h-4 text-[var(--neu-success)]" /> Sample Test Cases
                    </div>
                    <div className="space-y-2">
                        {sampleCases.map((tc, i) => (
                            <div key={i} className="p-3 rounded-lg border neu-inset border-[var(--neu-divider)] text-xs font-mono">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div className="neu-raised p-2 rounded border border-gray-100 dark:border-white/5 text-gray-600 dark:text-gray-400 whitespace-pre-wrap">
                                            {tc.input || '(empty)'}
                                        </div>
                                    </div>
                                    <div>
                            ...
                            return (
                                <div
                                    ...
                                    className={`p-3 rounded-lg border ${isPassed
                                        ? 'bg-[var(--neu-success-soft)] dark:bg-[var(--neu-success-soft)] border-emerald-200 dark:border-emerald-500/20'
                                        : 'bg-red-50 dark:bg-[var(--neu-danger-soft)] border-red-200 dark:border-red-500/20'
                                        }`}
                                >
                                    <div className="flex justify-between items-center mb-2">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`text-xs font-bold ${isPassed ? 'text-emerald-700 dark:text-[var(--neu-success)]' : 'text-red-700 dark:text-[var(--neu-danger)]'}`}>
                                                {tc.name}
                                            </span>
                                        </div>
                                        <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${isPassed ? 'bg-[var(--neu-success-soft)] dark:bg-[var(--neu-success-soft)] text-emerald-700 dark:text-[var(--neu-success)]' : 'bg-[var(--neu-danger-soft)] dark:bg-[var(--neu-danger-soft)] text-red-700 dark:text-[var(--neu-danger)]'}`}>
                                            {isPassed ? 'Passed' : 'Failed'}
                                        </span>
                                    </div>

                                    {executionResult && (
                                        <div className="text-[10px] grid grid-cols-2 gap-2 neu-raised p-2 rounded font-mono">
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <span className={`break-all font-bold ${isPassed ? 'text-[var(--neu-success)] dark:text-[var(--neu-success)]' : 'text-[var(--neu-danger)] dark:text-[var(--neu-danger)]'}`}>
                ...
                {hasHistory ? (
                    <div className="relative pl-6 space-y-6 before:absolute before:inset-y-2 before:left-2 before:w-[2px] before:bg-gray-200 dark:before:bg-[var(--card)]/10 before:rounded-full">
                        {sub.history.map((h, i) => {
                            ...
                            return (
                                <div key={i} className="relative">
                                    <div className={`absolute -left-[1.375rem] top-2 w-3.5 h-3.5 rounded-full border-[3px] border-white dark:border-[#1A1F2E] ${isSuccess ? 'bg-[var(--neu-success-soft)]' : 'bg-amber-500'}`} />
                                    <div className={`p-4 rounded-xl border transition-colors ${isLatest ? 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] border-blue-200 dark:border-blue-500/20' : 'neu-raised border-gray-100 dark:border-white/5'}`}>
                                        <div className="flex items-center justify-between mb-3 border-b border-gray-100 dark:border-white/5 pb-3">
                                            <div className="flex items-center gap-3">
                                                <span className={`text-xs font-black uppercase tracking-wider ${isSuccess ? 'text-[var(--neu-success)] dark:text-[var(--neu-success)]' : 'text-[var(--neu-warn)] dark:text-[var(--neu-warn)]'}`}>
                                                    {h.status || (isSuccess ? 'Success' : 'Failure')}
                                                </span>
                                                <span className="text-[10px] text-gray-400 font-mono tracking-wider">
                                                    {h.timestamp ? new Date(h.timestamp).toLocaleString() : 'N/A'}
                                                </span>
                                                {isLatest && <span className="text-[9px] bg-[var(--neu-achieve-soft)] text-white px-2 py-0.5 rounded-full font-bold uppercase tracking-widest shadow-sm shadow-blue-500/40">Latest Submission</span>}
                                            </div>
                                        </div>
                    ...
                    <div className="p-4 rounded-xl border neu-raised border-gray-100 dark:border-white/5">
            ...
            <div className="space-y-2">
                <div className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                    <Code className="w-4 h-4" /> Submitted Code (This Attempt)
                </div>
...
const MetricRow = ({ label, value, isWarning, suffix = '' }) => (
    <div className={`flex items-center justify-between p-3 rounded-lg ${isWarning ? 'bg-[var(--neu-danger-soft)] border border-red-500/20' : 'bg-transparent border border-gray-100 dark:border-white/5'}`}>
