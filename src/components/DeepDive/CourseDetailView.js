  ...
  <div className="p-4 space-y-4 animate-pulse">
    {[1, 2, 3].map(i => (
      <div key={i} className="space-y-2">
        <div className="h-12 neu-inset rounded-xl" />
        <div className="ml-6 space-y-1">
          <div className="h-8 neu-inset rounded-lg w-4/5" />
          <div className="h-8 neu-inset rounded-lg w-3/4" />
        </div>
      </div>
    ))}
  </div>
...
const ProgressRing = ({ progress, size = 32, strokeWidth = 3 }) => {
  ...
  return (
    <svg width={size} height={size} className="transform -rotate-90">
      ...
      />
      <circle
        className="text-[var(--neu-success)] transition-all duration-500"
        strokeWidth={strokeWidth}
        strokeLinecap="round"
        stroke="currentColor"
        fill="transparent"
...
const ContentTypeBadge = ({ type, className = "" }) => {
  const config = {
    'mcq': { bg: 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)]', text: 'text-violet-700 dark:text-violet-300', label: 'MCQ' },
    'mcq-group': { bg: 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)]', text: 'text-violet-700 dark:text-violet-300', label: 'MCQs' },
    'coding': { bg: 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)]', text: 'text-blue-700 dark:text-blue-300', label: 'Code' },
    'coding-group': { bg: 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)]', text: 'text-blue-700 dark:text-blue-300', label: 'Coding' },
    'video': { bg: 'bg-rose-100 dark:bg-rose-500/20', text: 'text-rose-700 dark:text-rose-300', label: 'Video' },
    'pdf': { bg: 'bg-[var(--neu-warn-soft)] dark:bg-amber-500/20', text: 'text-amber-700 dark:text-amber-300', label: 'Notes' },
  };
...
const SearchBar = ({ value, onChange, onClear }) => (
  <div className="relative">
    ...
    <input
      ...
      className="w-full pl-9 pr-8 py-2.5 neu-inset rounded-xl text-sm 
    ...
    {value && (
      <button
        onClick={onClear}
        className="absolute right-2 top-1/2 -translate-y-1/2 p-1 hover:bg-[var(--neu-dark)] rounded-full transition-colors"
      >
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${copied
        ? 'bg-[var(--neu-success-soft)] text-white'
        : 'bg-[var(--card)]/10 hover:bg-[var(--card)]/20 text-gray-300'
        } ${className}`}
    >
...
export default function CourseDetailView({ course, onBack }) {
    ...
    return (
      <button
        ...
        className={`group w-full text-left p-2.5 rounded-xl text-xs flex items-center gap-2.5 
                  ...
                  ${isSelected
            ? 'bg-[var(--neu-accent)] text-white shadow-lg shadow-blue-500/25 scale-[1.02]'
            : 'hover:bg-[var(--neu-dark)] text-gray-600 dark:text-gray-400'
          }`}
      >
        <div className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center transition-all
                      ${isSelected ? 'bg-[var(--card)]/20' : 'neu-inset'}`}>
  ...
  const MainContentArea = ({ content }) => {
    ...
    if (!content) {
      return (
        <div className="h-full flex flex-col items-center justify-center text-gray-400 p-8">
          <div className="relative">
            <div className="w-32 h-32 bg-[var(--neu-achieve-soft)] 
            ...
            <div className="absolute -top-2 -right-2 w-8 h-8 bg-[var(--neu-achieve-soft)] rounded-full 
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <kbd className="px-3 py-1.5 neu-inset rounded-lg text-xs font-mono">
              B
            </kbd>
    ...
    //             alt="Question figure"
    //             className="w-full h-auto max-h-80 object-contain neu-raised"
    //           />
    //         </div>
    //       )}

    //       {/* Question Card */}
    //       <div className="neu-raised rounded-2xl p-8 shadow-sm 
    ...
    //             let optionClass = 'neu-inset border-[var(--neu-divider)]';

    //             if (showResult) {
    //               if (isCorrect) {
    //                 optionClass = 'bg-[var(--neu-success-soft)] dark:bg-[var(--neu-success-soft)] border-emerald-500 ring-2 ring-emerald-500/20';
    //               } else if (isSelected && !isCorrect) {
    //                 optionClass = 'bg-red-50 dark:bg-[var(--neu-danger-soft)] border-red-500';
    //               }
    //             } else if (isSelected) {
    //               optionClass = 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] border-blue-500 ring-2 ring-blue-500/20';
    //             }
    ...
    //                         : isSelected
    //                           ? 'bg-[var(--neu-achieve-soft)] text-white'
    //                           : 'neu-inset text-gray-500 dark:text-gray-400'
    //                     }`}>
    //                     {showResult && isCorrect ? (
    ...
    //                        dark:bg-[var(--card)]/10 text-gray-600 dark:text-gray-400 
    //                        hover:bg-gray-200 dark:hover:bg-[var(--card)]/20 transition-colors text-sm font-medium"
    //             >
              ...
              {item.options?.map((opt, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`w-full p-4 rounded-xl border-2 text-left transition-all duration-300
                          ${isCorrect
                        ? 'bg-[var(--neu-success-soft)] dark:bg-[var(--neu-success-soft)] border-emerald-500 ring-2 ring-emerald-500/20'
                        : 'neu-inset border-[var(--neu-divider)]'
                      }`}
                  >
                    <div className="flex items-start gap-4">
                      <div className={`shrink-0 w-9 h-9 rounded-lg flex items-center justify-center 
                      ...
                      {isCorrect && (
                        <span className="px-2 py-1 rounded-md bg-[var(--neu-success-soft)] text-white text-xs font-bold uppercase">
                          Answer
                        </span>
                      )}
                    </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in-50 slide-in-from-bottom-4 duration-500">
          ...
          <div className="neu-raised rounded-2xl p-6 md:p-8 
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2.5 bg-[var(--neu-accent)] 
          ...
          {(item.whitelist?.length > 0 || item.blacklist?.length > 0) && (
            <div className="mt-6 pt-6 border-t border-gray-100 dark:border-white/5 space-y-4">
              {item.whitelist?.length > 0 && (
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-[var(--neu-success)] dark:text-[var(--neu-success)] mb-2 flex items-center gap-2">
                    <CheckCircle className="w-3.5 h-3.5" /> Allowed Terms (Whitelist)
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {item.whitelist.map((term, idx) => (
                      <span key={idx} className="px-2 py-1 rounded-md bg-[var(--neu-success-soft)] dark:bg-[var(--neu-success-soft)] text-emerald-700 dark:text-emerald-300 text-xs font-mono border border-emerald-100 dark:border-emerald-500/20">
                        {term}
                      </span>
                    ))}
                  </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {[
                    ...
                    <div key={i} className="neu-raised rounded-xl 
                                        
                                        overflow-hidden shadow-sm">
                      <div className="px-4 py-2 neu-flat 
                                  border-b border-gray-100 dark:border-white/5 
                                  flex justify-between items-center">
                        <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded
                                      ${tc.variant === 'sample'
                            ? 'bg-[var(--neu-achieve-soft)] text-blue-700 dark:bg-[var(--neu-achieve-soft)] dark:text-blue-300'
                            : 'bg-[var(--neu-achieve-soft)] text-violet-700 dark:bg-[var(--neu-achieve-soft)] dark:text-violet-300'
                          }`}>
                          {tc.variant} #{i + 1}
                        </span>
                      </div>
                      <div className="p-4 grid grid-cols-2 gap-4 text-xs font-mono">
                        <div>
                          ...
                          <div className="p-2 neu-flat rounded 
                                     ">
                            {tc.input}
                          </div>
                        </div>
        ...
        <>
          ...
          {pdfExpanded && (
            <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl 
              ...
              <div className="w-full h-full neu-raised rounded-2xl 
                        shadow-2xl flex flex-col overflow-hidden">
                <div className="flex items-center justify-between p-4 border-b 
                          border-[var(--neu-divider)] neu-flat">
                  <div className="flex items-center gap-4">
                    <div className="p-2 bg-[var(--neu-warn-soft)] dark:bg-amber-500/20 
                              text-[var(--neu-warn)] dark:text-[var(--neu-warn)] rounded-lg">
                        ...
                        {content.pdfs.map((pdf, idx) => (
                          <button
                            ...
                            className={`px-2 py-0.5 rounded text-xs font-medium transition-all
                                ...
                                : 'neu-inset text-gray-600 dark:text-gray-400'
                              }`}
                          >
                            {pdf.label}
                          </button>
                  ...
                  <button
                    ...
                    className="p-2 hover:bg-[var(--neu-dark)] 
                         rounded-full transition-colors"
                  >
                    <Minimize2 className="w-6 h-6" />
                  </button>
                </div>
                <div className="flex-1 neu-inset p-2">
                  <iframe
                    src={content.pdfs[activePdfIndex].url}
                    className="w-full h-full rounded-xl bg-[var(--card)]"
                    title="PDF Viewer"
                  />
                </div>
              </div>
          ...
          <div className="animate-in fade-in-50 duration-500">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                {content.pdfs.map((pdf, idx) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold transition-all
                        ...
                        : 'neu-raised text-gray-600 dark:text-gray-400 hover:bg-[var(--neu-dark)]'
                      }`}
                  >
                    {pdf.label}
                  </button>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[var(--card)] 
                     dark:bg-[var(--card)]/5 text-gray-600 dark:text-gray-400 
                     hover:bg-[var(--neu-dark)] transition-all text-xs font-medium"
              >
            ...
            <div
              className="neu-raised rounded-2xl shadow-sm overflow-hidden"
    ...
    // ─────────────────────────────────────────────────────────────────────────
    const renderVideo = () => (
      <div className="flex flex-col items-center justify-center p-16 bg-[var(--card)] 
        ...
        <a
          ...
          className="px-8 py-3.5 rounded-full bg-[var(--neu-accent)] 
      ...
      return (
        <div className="w-64 shrink-0 hidden xl:block">
          <div className="sticky top-6 space-y-4">
            <div className="p-5 neu-raised border border-gray-200 
                ...
                {content.items.map((_, i) => (
                  <button
                    ...
                    className={`aspect-square rounded-lg text-xs font-bold transition-all 
                              ...
                              ${currentIndex === i
                        ? 'bg-[var(--neu-achieve-soft)] text-white shadow-lg ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-[#0f1523]'
                        : 'neu-inset text-gray-600 dark:text-gray-400 hover:bg-[var(--neu-dark)]'
                      }`}
                  >
                    {i + 1}
                  </button>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 p-2.5 flex items-center justify-center rounded-xl 
                           neu-inset 
                           hover:bg-[var(--neu-dark)] disabled:opacity-30 
        ...
        <div className={`mx-auto h-full ${isGroup ? 'max-w-[1400px] flex gap-8' : 'max-w-4xl'}`}>
          <div className="flex-1 min-w-0">
            ...
            <div className="sticky top-0 z-20 -mx-6 -mt-6 px-6 py-4 mb-8
                          ...
                          border-b border-[var(--neu-divider)] shadow-sm">
                ...
                {isGroup && (
                  <div className="flex items-center gap-2 xl:hidden">
                    <button
                      onClick={() => setCurrentIndex(c => Math.max(0, c - 1))}
                      disabled={currentIndex === 0}
                      className="p-2 rounded-lg hover:bg-[var(--neu-dark)] 
  ...
  return (
    <PortalWrapper>
    <div className="fixed inset-0 z-[120] flex flex-col neu-page 
      ...
      <header className="flex items-center justify-between p-4 px-6 border-b 
                       border-[var(--neu-divider)] neu-raised shrink-0">
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
            className="p-2 -ml-2 hover:bg-[var(--neu-dark)] 
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="p-2 rounded-xl hover:bg-[var(--neu-dark)] 
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Sidebar */}
        <aside className={`neu-raised border-r border-gray-200 
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
            {loading ? (
              ...
              <div className="p-6 text-center">
                ...
                <button
                  onClick={() => fetchCourseContent(true)}
                  className="px-4 py-2 bg-[var(--neu-danger-soft)] dark:bg-[var(--neu-danger-soft)] text-[var(--neu-danger)] 
                           dark:text-[var(--neu-danger)] rounded-lg text-sm font-medium 
                  ...
                  <div key={uIdx} className="overflow-hidden">
                    <button
                      ...
                      className={`w-full flex items-center justify-between p-3 rounded-xl 
                                ...
                                ${expandedUnits[uIdx]
                          ? 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] border-2 border-blue-200 dark:border-blue-500/30'
                          : 'hover:bg-[var(--neu-dark)] border-2 border-transparent'
                        }`}
                    >
                      <div className="flex items-center gap-3 min-w-0">
                    ...
                    {/* Sub-units */}
                    {expandedUnits[uIdx] && (
                      <div className="ml-4 pl-4 border-l-2 border-[var(--neu-divider)] 
                            ...
                            <div key={sIdx}>
                              <button
                                ...
                                className={`w-full text-left py-2.5 px-3 rounded-lg flex items-center 
                                    ...
                                    ? 'bg-[var(--neu-achieve-soft)] dark:bg-[var(--neu-achieve-soft)] border-l-2 border-blue-500'
                                    : 'hover:bg-[var(--neu-dark)]'
                                  }`}
                              >
                                {expandedLectures[subKey]
                                ...
                                <span className="text-[10px] text-gray-400 bg-gray-100 
                                              dark:bg-[var(--card)]/10 px-1.5 py-0.5 rounded-full">
                                  {items.length}
                                </span>
                              </button>
        ...
        {/* Main Content Area */}
        <main className="flex-1 neu-page relative overflow-hidden">
